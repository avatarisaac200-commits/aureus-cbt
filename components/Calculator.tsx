  ...
  return (
    <div className="fixed bottom-20 right-4 sm:bottom-24 sm:right-8 w-64 sm:w-72 bg-slate-900 border-2 border-slate-800 rounded-2xl shadow-2xl p-4 z-[60] text-white">
      <div className="flex justify-between items-center mb-3">
        <span className="text-xs font-black uppercase tracking-widest text-amber-500">Scientific Engine</span>
      ...
      <div className="grid grid-cols-4 gap-1.5 sm:gap-2">
        <button onClick={() => handleSci('sin')} className="bg-slate-800 p-2 rounded-lg text-xs font-black hover:bg-slate-700">SIN</button>
        <button onClick={() => handleSci('cos')} className="bg-slate-800 p-2 rounded-lg text-xs font-black hover:bg-slate-700">COS</button>
        <button onClick={() => handleSci('ln')} className="bg-amber-600/20 text-amber-500 p-2 rounded-lg text-xs font-black hover:bg-amber-600/30">LN</button>
        <button onClick={() => handleSci('exp')} className="bg-amber-600/20 text-amber-500 p-2 rounded-lg text-xs font-black hover:bg-amber-600/30">EXP</button>

        <button onClick={() => handleSci('log')} className="bg-slate-800 p-2 rounded-lg text-xs font-black hover:bg-slate-700">LOG</button>
        <button onClick={() => handleSci('sqrt')} className="bg-slate-800 p-2 rounded-lg text-xs font-black hover:bg-slate-700">{'\u221A'}</button>
        <button onClick={handleClear} className="bg-rose-600 p-2 rounded-lg text-xs font-black hover:bg-rose-500">AC</button>
