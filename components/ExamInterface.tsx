  ...
  if (view === 'lobby') {
    return (
      <div className="h-full w-full bg-slate-50 flex flex-col overflow-hidden">
        <header className="bg-slate-950 p-6 flex justify-between items-center border-b-4 border-amber-500 text-white shadow-xl safe-top">
          <div className="flex items-center gap-3">
            <img src={logo} className="w-10 h-10" alt="Logo" />
            <div>
              <h1 className="text-[10px] font-black uppercase tracking-widest text-amber-500 truncate max-w-[150px]">Aureus Medicos</h1>
              <p className="text-[8px] text-slate-400 font-bold uppercase truncate max-w-[120px]">{test.name}</p>
            </div>
          </div>
          <div className="bg-slate-900 border border-slate-800 px-4 py-1.5 rounded-xl text-amber-400 font-mono text-lg font-black">
            {hasStarted ? formatTime(timeRemaining) : "LOBBY"}
          </div>
        </header>

        <main className="flex-1 overflow-y-auto p-6 md:p-12 no-scrollbar safe-bottom">
          <div className="max-w-4xl mx-auto bg-white rounded-[2.5rem] shadow-sm border border-gray-200 p-8 md:p-12">
            <h2 className="text-xl font-black text-slate-950 mb-1 uppercase tracking-tight">Exam Structure</h2>
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-10">Aureus Medicos CBT Practice App</p>
                  ...
                  <button key={idx} onClick={() => enterSection(idx)} disabled={isCompleted} className={`w-full flex justify-between items-center p-6 rounded-2xl border-2 transition-all ${isCompleted ? 'bg-slate-50 border-slate-100 opacity-50' : 'bg-white border-gray-100 hover:border-amber-500'}`}>
                    <div className="flex items-center gap-6">
                      ...
                      <div className="text-left">
                        <h3 className="font-black text-slate-900 text-sm uppercase truncate max-w-[180px]">{section.name}</h3>
                        <p className="text-[9px] text-slate-400 font-bold uppercase">{section.questionIds.length} Questions</p>
                      </div>
                    </div>
                    <span className={`text-[10px] font-black px-4 py-2 rounded-full uppercase ${isCompleted ? 'bg-slate-200 text-slate-400' : 'bg-amber-100 text-amber-600'}`}>{isCompleted ? 'Locked' : 'Start'}</span>
                  </button>
                );
              })}
            </div>
            <div className="mt-12 pt-10 border-t border-gray-100 flex flex-col md:flex-row gap-6 justify-between items-center text-center md:text-left">
              <button onClick={() => { if(window.confirm('Terminate exam?')) onExit(); }} className="text-[10px] font-black text-slate-400 hover:text-red-500 uppercase tracking-widest">Quit Practice</button>
              <div>
                <button onClick={finalSubmit} disabled={completedSections.length < test.sections.length || isFinishing} className="w-full md:w-auto px-12 py-5 bg-slate-950 text-amber-500 rounded-2xl text-[10px] font-black uppercase tracking-widest disabled:opacity-20 shadow-2xl active:scale-95 transition-all">End Session</button>
                <p className="text-[8px] text-slate-300 uppercase font-black tracking-widest mt-4">Aureus Medicos CBT Practice App</p>
              </div>
            </div>
          </div>
        </main>
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 select-none overflow-hidden">
      <header className="bg-slate-950 text-white px-6 py-4 flex justify-between items-center border-b-4 border-amber-500 z-30 shrink-0 safe-top">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:block">
            <h1 className="text-[10px] font-black uppercase tracking-widest text-amber-500">Aureus Medicos</h1>
            <p className="text-[8px] text-slate-400 font-bold uppercase truncate max-w-[200px]">{test.name}</p>
          </div>
          <span className="text-[10px] font-black uppercase text-amber-500 sm:hidden">Item {currentQuestionIndex + 1}</span>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        <main className="flex-1 flex flex-col p-4 md:p-8 overflow-hidden">
          <div className="flex-1 bg-white rounded-[2rem] md:rounded-[3rem] shadow-sm border border-gray-200 overflow-y-auto p-8 md:p-14 no-scrollbar">
            ...
            <div className="text-lg md:text-2xl font-bold text-slate-900 mb-12 leading-relaxed"><ScientificText text={currentQuestion?.text || "Loading..."} /></div>
            <div className="space-y-4">
        ...
        <aside className={`fixed inset-y-0 right-0 w-80 bg-white border-l border-gray-200 z-40 transform transition-transform duration-300 md:relative md:translate-x-0 ${showNav ? 'translate-x-0' : 'translate-x-full'} shadow-2xl md:shadow-none`}>
          <div className="h-full flex flex-col">
            ...
            <div className="p-6 bg-slate-50 border-t border-gray-100 space-y-3">
              <button onClick={() => { setShowCalculator(!showCalculator); setShowNav(false); }} className="w-full py-4 bg-slate-950 text-amber-500 rounded-2xl text-[10px] font-black uppercase tracking-widest">Calculator</button>
              <p className="text-[7px] text-center font-black text-slate-300 uppercase tracking-widest">Aureus Medicos Practice App</p>
            </div>
          </div>
        </aside>
      </div>
      ...
      <footer className="bg-white border-t border-gray-200 p-6 md:p-8 px-8 md:px-12 flex justify-between items-center z-20 shrink-0 safe-bottom">
         <button onClick={() => setView('lobby')} className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-900 transition-colors">Menu</button>
         <div className="flex gap-3">
           <button onClick={() => setCurrentQuestionIndex(prev => Math.max(0, prev - 1))} disabled={currentQuestionIndex === 0} className="px-6 py-3 border border-slate-200 rounded-xl text-[10px] font-black uppercase disabled:opacity-20 active:scale-95 transition-all">Back</button>
           <button onClick={() => setCurrentQuestionIndex(prev => Math.min(activeSection.questionIds.length - 1, prev + 1))} disabled={currentQuestionIndex === activeSection.questionIds.length - 1} className="px-6 py-3 border border-slate-200 rounded-xl text-[10px] font-black uppercase disabled:opacity-20 active:scale-95 transition-all">Next</button>
           <button onClick={handleSectionSubmit} className="ml-4 px-10 py-4 bg-amber-500 text-slate-950 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">Submit Section</button>
         </div>
      </footer>
