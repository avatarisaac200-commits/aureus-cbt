...
const ExamInterface: React.FC<ExamInterfaceProps> = ({ test, user, onFinish, onExit }) => {
  ...
  const handleSectionSubmit = () => {
    ...
    if (window.confirm("Finish this section? You won't be able to change your answers for this section.")) {
  ...
  const finalSubmit = () => {
    if (window.confirm("Finish the entire test?")) {
      setIsFinishing(true);
      ...
      <div className="h-full w-full bg-slate-50 flex flex-col overflow-hidden safe-top">
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-12 no-scrollbar safe-bottom">
          <div className="max-w-4xl mx-auto bg-white rounded-[2rem] shadow-xl border border-slate-100 p-8 md:p-12">
            ...
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-10">Please complete all sections to finish the test.</p>
              ...
              {test.sections.map((section, idx) => {
                ...
                return (
                  <button key={idx} onClick={() => enterSection(idx)} disabled={isCompleted} className={`w-full flex justify-between items-center p-6 rounded-2xl border-2 transition-all ${isCompleted ? 'bg-slate-50 border-slate-100 opacity-50' : 'bg-white border-slate-100 hover:border-amber-500'}`}>
                    ...
                    <span className={`text-[9px] font-bold px-4 py-2 rounded-xl uppercase tracking-widest transition-all ${isCompleted ? 'bg-slate-100 text-slate-400' : 'bg-amber-100 text-amber-600'}`}>{isCompleted ? 'Finished' : 'Start Now'}</span>
                  </button>
                );
              })}
            </div>
            <div className="mt-12 pt-8 border-t border-slate-50 flex flex-col md:flex-row gap-6 justify-between items-center">
              <button onClick={() => { if(window.confirm('Stop the test and leave?')) onExit(); }} className="text-[10px] font-bold text-slate-400 hover:text-red-500 uppercase tracking-widest transition-colors">Quit Test</button>
              <button onClick={finalSubmit} disabled={completedSections.length < test.sections.length || isFinishing} className="w-full md:w-auto px-10 py-4 bg-slate-950 text-amber-500 rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-xl hover:bg-slate-900 transition-all disabled:opacity-30">Submit Entire Test</button>
            </div>
          </div>
        </main>
      </div>
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 select-none overflow-hidden safe-top">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        <main className="flex-1 flex flex-col p-4 md:p-8 overflow-hidden">
          <div className="flex-1 bg-white rounded-[2rem] md:rounded-[3rem] shadow-sm border border-slate-100 overflow-y-auto p-8 md:p-12 no-scrollbar">
            ...
            <div className="text-lg md:text-2xl font-bold text-slate-900 mb-12 leading-tight"><ScientificText text={currentQuestion?.text || "Loading question..."} /></div>
            <div className="space-y-4">
        ...
        <aside className={`fixed inset-y-0 right-0 w-72 bg-white border-l border-slate-100 z-40 transform transition-transform duration-300 md:relative md:translate-x-0 ${showNav ? 'translate-x-0' : 'translate-x-full'} shadow-2xl md:shadow-none`}>
          <div className="h-full flex flex-col">
            <div className="p-6 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
              <h3 className="text-[10px] font-bold text-slate-950 uppercase tracking-widest">Question Map</h3>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100">
              <button onClick={() => { setShowCalculator(!showCalculator); setShowNav(false); }} className="w-full py-4 bg-slate-950 text-amber-500 rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-lg">Open Calculator</button>
            </div>
          </div>
        </aside>
      </div>
      ...
      <footer className="bg-white border-t border-slate-100 p-6 flex flex-col sm:flex-row gap-4 justify-between items-center z-20 shrink-0 safe-bottom">
         <button onClick={() => setView('lobby')} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-amber-600 transition-colors">Go Back to Menu</button>
         <div className="flex gap-2 w-full sm:w-auto">
           <button onClick={() => setCurrentQuestionIndex(prev => Math.max(0, prev - 1))} disabled={currentQuestionIndex === 0} className="flex-1 sm:flex-none px-6 py-3 border-2 border-slate-100 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-slate-50 disabled:opacity-30">Previous</button>
           ...
           <button onClick={handleSectionSubmit} className="flex-1 sm:flex-none px-8 py-3 bg-amber-500 text-slate-950 rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-md">Finish Section</button>
         </div>
      </footer>
