...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, initialTab = 'questions', onLogout, onSwitchToStudent }) => {
  ...
  // Test Builder State (Section-by-Section)
  ...
  // QUOTA PROTECTION: Only load the 50 most recent questions initially.
  useEffect(() => {
  ...
  // DUPLICATE CLEANUP ALGORITHM
  const runBankCleanup = async () => {
    if (!window.confirm("This will find and delete identical questions to save space. Continue?")) return;
    setLoading(true);
    try {
      const snap = await getDocs(collection(db, 'questions'));
      const all = snap.docs.map(d => ({ ...d.data(), id: d.id } as Question));
      
      const seenTexts = new Map<string, string>(); // normalizedText -> questionId
      ...
      if (duplicatesToDelete.length > 0) {
        ...
        await batch.commit();
        alert(`Cleanup finished. Removed ${duplicatesToDelete.length} duplicates.`);
      } else {
        alert("Your question bank is already clean!");
      }
    } catch (err) {
      console.error(err);
        ...
        config: {
          ...
          responseSchema: {
            ...
            items: {
              ...
              required: ["subject", "topic", "text", "options", "correctAnswerIndex"],
            },
          },
        },
      });
